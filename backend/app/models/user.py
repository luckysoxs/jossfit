...
from sqlalchemy import String, Integer, Float, Enum, DateTime, Boolean, JSON, func
...
class User(Base):
    ...
    pathologies: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    medications: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    mobility_limitations: Mapped[dict | None] = mapped_column(JSON, nullable=True)
